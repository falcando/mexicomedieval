...
function SpotifyEpisodeGroups({ guests, showTitle = true }: { guests: readonly SpotifyGuestSpot[], showTitle?: boolean }) {
  ...
  const spotifySeriesGroupsWithArt = useMemo(() => {
    ...
    return groups.map((group, gi) => ({
      ...
      episodes: group.episodes.map((item, j) => {
        ...
        return {
          item,
          image: CARD_IMAGES[group.series as keyof typeof CARD_IMAGES],
          altKey: cardAltKeys[linear % cardAltKeys.length],
        };
      }),
    }));
  }, [guests]);
      ...
      {spotifySeriesGroupsWithArt.map((group, groupIndex) => (
        ...
        >
          <div className="mb-12">
            ...
            {showTitle && (
              <h3 className="font-headline text-3xl font-bold text-primary md:text-4xl capitalize">
                {group.series.replace(/-/g, ' ')}
              </h3>
            )}
...
export function PodcastIndexPage() {
  const { t } = useTranslations();
  const [mmQuery, diasQuery, othersQuery] = usePodcastCatalogSectionsQueries();

  const mexicoMedievalAudioCards = useMemo(() => {
    const alts = cardAltKeys.map((k) => t(k));
    const items = mmQuery.data?.appleEpisodes ?? [];
    return items.map((item, i) => {
      const image = CARD_IMAGES['mexico-medieval'];
  ...
  return (
    <PageContainer title={t("podcastPage.titleLine")} subtitle={t("podcastPage.subtitle")}>
      ...
      <section
        id="podcast-dias-con-la-garra-y-ale-garibay"
        className="mb-32 scroll-mt-24 border-t border-outline-variant/15 pt-16"
      >
        <div className="mx-auto max-w-screen-2xl px-8">
          <div className="mb-12">
            <h2 className="font-headline text-4xl font-bold text-primary">
              {t("podcastPage.sectionDiasGarraTitle")}
            </h2>
            <p className="mt-2 text-on-surface-variant">
              {t("podcastPage.sectionDiasGarraSubtitle")}
              </p>
              <p className="mt-2 text-on-surface-variant">
                {t("podcastPage.sectionDiasGarraBody")}
              </p>
          </div>
          {diasQuery.isPending && !diasQuery.data && (
            <p className="mb-8 text-center text-on-surface-variant">
              {t("podcastPage.listLoading")}
            </p>
          )}
          ...
          <div aria-busy={anyFetching}>
            <SpotifyEpisodeGroups guests={diasQuery.data?.spotifyGuests ?? []} showTitle={false} />
          </div>
        </div>
      </section>

      <section id="podcast-colaboraciones" className="mb-32 scroll-mt-24">
        <div className="mx-auto max-w-screen-2xl px-8">
          <div className="mb-12">
            <h2 className="font-headline text-4xl font-bold text-primary">
              {t("podcastPage.sectionColaboracionesTitle")}
            </h2>
            <p className="mt-2 text-on-surface-variant">
              {t("podcastPage.collaborationsSubtitle")}
            </p>
            <p className="mt-2 text-on-surface-variant">
              {t("podcastPage.collaborationsBody")}
            </p>
          </div>
          {othersQuery.isPending && !othersQuery.data && (
            <p className="mb-8 text-center text-on-surface-variant">
              {t("podcastPage.listLoading")}
            </p>
          )}
          ...
          <div aria-busy={anyFetching}>
            <SpotifyEpisodeGroups guests={othersQuery.data?.spotifyGuests ?? []} />
          </div>
        </div>
      </section>
