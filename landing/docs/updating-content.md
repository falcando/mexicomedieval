...
All edits belong on the **`dev`** branch. See [Deployment and branches](deployment.md) for why you should never edit **`main`** manually.
...
- After you save your edits on **`dev`**, CI runs a build automatically. If you have the project set up locally, run **`pnpm run build`** (from the `landing` folder) once to confirm everything still works before pushing. If that command reports an error, share the message with whoever maintains the code.
...
1. **Work on `dev`** — Never commit content changes to **`main`**; that branch is replaced automatically by the publish workflow.
2. **Make a backup** — Use “Save as” or work on a copy branch in Git if your team uses version control.
3. **Match the punctuation** — Items are separated by commas. Text must stay inside straight **quotes** `"like this"`. Curly quotes from Word `"like this"` can break the file; use the editor’s straight quotes.
4. **Copy, don’t invent the shape** — Find the last complete entry in the list (from `{` to `},`), duplicate it, then change the content. That way commas, braces, and field names stay correct.
...
- [ ] Changes are on the **`dev`** branch (not `main`).
...
- [ ] Run **`pnpm run build`** locally if you can, or rely on CI after you push to `dev`.
...
- **`AGENTS.md`** (in the `landing` folder) — notes for people working with the code editor and automated tools.
